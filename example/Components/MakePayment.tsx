...
import { launchImageLibrary } from 'react-native-image-picker'
import * as RNFS from 'react-native-fs'
import FaceSDK, { Enum, FaceCaptureResponse, MatchFacesResponse, MatchFacesRequest, MatchFacesImage, MatchFacesSimilarityThresholdSplit, RNFaceApi, LivenessNotification, VideoEncoderCompletion, InitializationConfiguration } from '@regulaforensics/react-native-face-api'
import { getAllImages } from '../Database/Database'
interface IProps {
}
interface ImageObject {
...
export default class MakePayment extends React.Component<IProps, IState> {
 ...
 componentDidUpdate(prevProps, prevState) {
  ...
  matchFaces(index=0) {
    if (image1 == null || image1.bitmap == null || image1.bitmap == "" || this.state.allImages.length === 0 || index >= this.state.allImages.length || this.state.match) {
       return;
    }
    this.setState({ similarity: "Processing...",isLoading:true });

    // Iterate over allImages to find a match
       const image2 = new MatchFacesImage();
       image2.bitmap = this.state.allImages[index].uri; // Assuming the URI is directly usable
       image2.imageType = Enum.ImageType.PRINTED; // Adjust the image type as necessary
         ...
         FaceSDK.matchFacesSimilarityThresholdSplit(JSON.stringify(response!.results), 0.75, str => {
           ...
           if (split.matchedFaces!.length > 0) {
            console.log('match found');
             console.log(`Respective number: ${this.state.allImages[index].number}`); 
             this.setState({upi:this.state.allImages[index].upi,number:this.state.allImages[index].number});
            this.setState({match:true,isLoading:false})
            //  this.setState({ similarity: split.matchedFaces!.length > 0 ? ((split.matchedFaces![0].similarity! * 100).toFixed(2) + "%") : "error" })
           }
           else {
            // No match found, proceed to the next image
            this.matchFaces(index + 1);
          }
         }, e => {
           console.error('Error in matchFacesSimilarityThresholdSplit:', e);
         });
    ...
    if (this.state.match) {
      return (
        <SafeAreaView style={styles.container}>
             ...
             <Image source={require('../images/upi.png')} style={{ height: 50, width: 50,alignSelf:'center',padding:5}} />
        <Text>Paying</Text>
        <Text>{this.state.number}</Text>
      <Text >UPI ID: {this.state.upi}</Text>
